...
COMPOSE_DIR="/volume1/docker/kimitter"
LOG_FILE="$COMPOSE_DIR/backend-sync.log"
...
# 새 이미지 pull
docker compose pull backend >> "$LOG_FILE" 2>&1

# 새 이미지 digest 비교
NEW_DIGEST=$(docker inspect --format='{{index .RepoDigests 0}}' "$IMAGE" 2>/dev/null)

if [ "$OLD_DIGEST" != "$NEW_DIGEST" ]; then
  log "New image detected. Restarting backend..."
  docker compose up -d backend >> "$LOG_FILE" 2>&1
  log "Backend restarted with new image."
else
  log "No update."
fi
